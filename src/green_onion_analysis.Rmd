...
```{r load-clean}
# Load
raw <- read_csv("data/final_dataset.csv", show_col_types = FALSE) 
...
The cleaned dataset (`df`) contains **59 rows × 15 columns**, with the following variables:

| Variable | Type | Meaning | Possible Values / Range |
|----------|------|---------|-------------------------|
| **lab** | factor | Lab where the plant was grown | `"Chandler"`, `"An"`, `"Dhruba"` |
...
| **leaf_color** | factor | Visual rating of leaf color | `"vibrant green"`, `"dark green"`, `"yellowish/brownish"` |
| **bulb_color** | factor | Whether the lower bulb area is clear and white | `"clear & white"`, `"not clear/white"` |
| **tip** | numeric | Length of dried/brown leaf tip (mm) | ~0.5 – 11.0 mm |
| **dead_leaf** | factor | Presence of dried/dead leaf tissue | `"Yes"` or `"No"` |
| **freshness** | factor | Rule-based computed freshness | `"Yes"` or `"No"` |
| **label_grp** | factor | Treatment group (environment condition) | `"GL"`, `"DN"`, `"DW"`, `"SN"`, `"SW"`, `"FN"` |
| **label_serial** | integer | Serial number within treatment group | 1, 2, 3, 4, ... |
...
The derived column (`freshness_calc`) is used only as a **consistency check** against the originally recorded `freshness` field.
...
## 3.2. Circumference by Lab × Label Group
...
**Interpretation** Across labs, the proportion of plants classified as “fresh” varies, with An showing the highest share of fresh samples and Chandler the lowest, suggesting differences in growing conditions or handling across locations.
